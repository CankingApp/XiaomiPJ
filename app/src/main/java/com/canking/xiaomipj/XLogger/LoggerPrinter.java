...
/**
 * XLogger is a wrapper for logging utils
...
final class LoggerPrinter implements Printer {
   ...
   *
   * @param tag is the given string which will be used in XLogger
   */
  @Override
  ...
  private int getStackOffset(StackTraceElement[] trace) {
    for (int i = MIN_STACK_OFFSET; i < trace.length; i++) {
      ...
      if (!name.equals(LoggerPrinter.class.getName()) && !name.equals(XLogger.class.getName())) {
        return --i;
      }
    }
    return -1;
